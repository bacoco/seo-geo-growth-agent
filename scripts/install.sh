...
cp -R "$ROOT/examples/reference-audit" "$DEST/examples/"
cp -R "$ROOT/examples/preprod-gated-audit" "$DEST/examples/"
mkdir -p "$DEST/scripts"
cp "$ROOT/scripts/seo_geo_audit.py" "$DEST/scripts/"
cp "$ROOT/scripts/run_full_audit.py" "$DEST/scripts/"
cp "$ROOT/scripts/validate_audit_report.py" "$DEST/scripts/"
cp "$ROOT/scripts/generate_html_audit_report.py" "$DEST/scripts/"
cp "$ROOT/scripts/generate_ai_layer_package.py" "$DEST/scripts/"
cp "$ROOT/scripts/generate_owner_data_request.py" "$DEST/scripts/"
cp "$ROOT/scripts/generate_ard_catalog.py" "$DEST/scripts/"
cp "$ROOT/scripts/validate_ard_catalog.py" "$DEST/scripts/"
cp "$ROOT/scripts/check_ard_readiness.py" "$DEST/scripts/"
cp "$ROOT/scripts/compare_audit_reports.py" "$DEST/scripts/"
cp "$ROOT/scripts/generate_geo_citation_panel.py" "$DEST/scripts/"
cp "$ROOT/scripts/runtime_config.py" "$DEST/scripts/"
