...
def slug(value: Any, fallback: str = "resource") -> str:
    clean = re.sub(r"[^a-zA-Z0-9._-]+", "-", text(value).strip()).strip("-").lower()
    return clean or fallback


def publisher_domain(payload: dict[str, Any]) -> str:
    parsed = urlparse(text(payload.get("canonical_url")))
    host = parsed.netloc or text(payload.get("site"), "example.com")
...
def representative_queries(audit: dict[str, Any], payload: dict[str, Any]) -> list[str]:
    candidates: list[str] = []
    for item in as_array(audit.get("representative_queries")):
        if item:
            candidates.append(text(item).strip())
    for item in payload.get("recommended_use", []):
        if item:
            candidates.append(text(item).strip())
    candidates.extend(
        [
            f"audit {payload['site']} for SEO and AI search readiness",
            f"generate a citation-safe /for-ai package for {payload['site']}",
        ]
    )
    unique: list[str] = []
    for candidate in candidates:
        ...
        if len(unique) >= 5:
            break
    return unique[:5] if len(unique) >= 2 else unique + ["check whether this site can be cited safely by AI assistants"]
...
def write_install_guide(path: Path, payload: dict[str, Any], include_ard: bool = False) -> None:
    ard_publish_step = (
        "6. If the owner confirms an agentic resource should be discoverable, publish `ai-catalog.json` as "
        "`/.well-known/ai-catalog.json` and optionally reference it with `<link rel=\"ai-catalog\">` or `Agentmap:`.\n"
        if include_ard
        else ""
    )
    ard_check_step = (
        "- `/.well-known/ai-catalog.json` is valid ARD JSON and represents a real skill, MCP server, A2A agent, or callable AI service.\n"
        if include_ard
        else ""
    )
...
{ard_check_step}- The visible human page and the AI files do not contradict each other.
        ...
        files.append(
            {
                ...
                "path": f"{PACKAGE_DIR}/ai-catalog.json",
...
def _entry_queries(entry: dict[str, Any], audit: dict[str, Any], payload: dict[str, Any]) -> list[str]:
    queries = [text(item).strip() for item in as_array(entry.get("representativeQueries")) if text(item).strip()]
    if len(queries) < 2 or len(queries) > 5:
        return representative_queries(audit, payload)
    return queries


def normalize_ard_entry(entry: dict[str, Any], audit: dict[str, Any], payload: dict[str, Any], index: int) -> dict[str, Any]:
    publisher = publisher_domain(payload)
    display_name = text(entry.get("displayName") or entry.get("name"), f"{payload['site']} AI resource")
    identifier = text(entry.get("identifier"))
    if not re.match(r"^urn:air:[A-Za-z0-9.-]+(?::[A-Za-z0-9][A-Za-z0-9._-]*){2,}$", identifier):
        identifier = f"urn:air:{publisher}:resource:{slug(display_name, 'resource')}-{index + 1}"

    normalized: dict[str, Any] = {
        "identifier": identifier,
        "displayName": display_name,
        "type": text(entry.get("type"), "application/ai-resource+json"),
        "description": text(
            entry.get("description"),
            "Owner-reviewed draft resource metadata generated from explicit audit evidence.",
        ),
        "representativeQueries": _entry_queries(entry, audit, payload),
        "metadata": {
            **(entry.get("metadata") if isinstance(entry.get("metadata"), dict) else {}),
            "spec_status": "ARD v0.9 draft",
            "owner_review": "required_before_publication",
        },
    }
    if entry.get("url"):
        normalized["url"] = text(entry.get("url"))
    elif entry.get("data") not in (None, ""):
        normalized["data"] = entry.get("data")
    else:
        normalized["url"] = payload["canonical_url"]
    for key in ("tags", "capabilities", "version", "updatedAt", "trustManifest"):
        if key in entry:
            normalized[key] = entry[key]
    return normalized


def ard_catalog_from_audit(audit: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any] | None:
    ard = audit.get("ard_readiness") if isinstance(audit.get("ard_readiness"), dict) else {}
    raw_entries = [item for item in as_array(ard.get("entries")) if isinstance(item, dict)]
    agentic_resources = [item for item in as_array(audit.get("agentic_resources")) if isinstance(item, dict)]
    include_ard = bool(raw_entries or agentic_resources or audit.get("include_ard_catalog"))
    if not include_ard:
        return None

    source_entries = raw_entries or agentic_resources
    if not source_entries:
        source_entries = [
            {
                "displayName": f"{payload['site']} AI-readable resource",
                "type": "application/geo-ai-layer+json",
                "url": payload["canonical_url"],
                "description": "Draft AI-readable site resource generated from explicit audit evidence.",
            }
        ]
    entries = [normalize_ard_entry(entry, audit, payload, index) for index, entry in enumerate(source_entries)]
    publisher = publisher_domain(payload)
    catalog = {
        "specVersion": "1.0",
        "host": {
            "displayName": payload["site"],
            "identifier": f"https://{publisher}",
            "documentationUrl": payload["canonical_url"],
        },
        "entries": entries,
        "metadata": {
            "spec_status": "ARD v0.9 draft",
    ...
    if errors:
        raise SystemExit("Generated ARD catalog is invalid:\n- " + "\n- ".join(errors))
    return catalog
...
def write_package(audit: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if ard_catalog:
        (package_dir / "ai-catalog.json").write_text(
